...
void HttpServer::register_routes() {
    ...
    server_.Get("/conversations", [this](const httplib::Request& req, httplib::Response& res) {
        handle_conversations(req, res);
    });
    server_.Delete(R"(/conversations/(\d+))",
                   [this](const httplib::Request& req, httplib::Response& res) {
                       handle_conversation_erase(req, res);
                   });
...
void HttpServer::handle_conversations(const httplib::Request&, httplib::Response& res) const {
    nlohmann::json conversations = nlohmann::json::array();
    for (const ninfer::ConversationSummary& conversation : service_->list_conversations()) {
        conversations.push_back(nlohmann::json{
            {"id", conversation.id},
            {"name", conversation.name},
            {"tokens", conversation.tokens},
            {"frontier", conversation.frontier},
        });
    }
    res.set_content(conversations.dump(), "application/json");
}

void HttpServer::handle_conversation_erase(const httplib::Request& req, httplib::Response& res) {
    ApiError error;
    error.type = "invalid_request_error";
    std::uint64_t id = 0;
    try {
        ...
        error.status  = 400;
        error.message = "conversation id is out of range";
        write_error(res, error);
        return;
    }
    try {
        if (!service_->erase_conversation(id)) {
            error.status  = 404;
            error.message = "no such conversation";
            write_error(res, error);
            return;
        }
        res.set_content(nlohmann::json{{"id", id}, {"erased", true}}.dump(),
                        "application/json");
